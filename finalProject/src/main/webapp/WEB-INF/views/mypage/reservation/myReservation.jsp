			...
			<table class="table">
				...
				<tbody id="stay_res_list">
					<c:forEach var="list" items="${dtos_stay}">
						...
						<c:if test="${time == 0}">
							<td><button class="btn btn-primary" onclick="stayResCancel('${list.reservation_no}')">예약취소</button></td>
						</c:if>
						<c:if test="${time == 1}">
						...
						<tr>
							<td colspan="10" style="text-align: center;">
								<c:if test="${blockStartNumber_stay!=1}">
									<input type="button" value="[◀◀]" onclick="getStayResList('${startPage_stay}')">
									<input type="button" value="[◀]" onclick="getStayResList('${blockStartNumber_stay-1}')">
								</c:if>
								<c:forEach var="n" begin="${blockStartNumber_stay}" end="${blockEndNumber_stay}">
								...
								<c:if test="${blockEndNumber_stay!=endPage_stay}">
									<input type="button" value="[▶]" onclick="getStayResList('${blockEndNumber_stay+1}')">
									<input type="button" value="[▶▶]" onclick="getStayResList('${endPage_stay}')">
								</c:if>
							</td>
						</tr>
				</tbody>
